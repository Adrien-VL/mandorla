...
class TrainConfig:
  cache_dir: str = "./data"
  out_dir: str = "./data/iter-curriculum-44m"
  resume: str | None = None

  # ~50M params, 12 layers (SoTA-comparable cluster)
  n_layers: int = 12
  n_heads: int = 8
  d_model: int = 512
  d_head: int = 64
  ...
  steps_per_phase: int = 4000      # 48K total steps
  ...
  beta2: float = 0.95
  grad_clip: float = 1.0
  batch_size: int = 64             # A100 has the memory; doubles tokens/step
  seq_len: int = 1024
...
def main(cfg: TrainConfig) -> None:
  ...
  # Build (or load cached) curriculum ordering: easiest chunks first.
  sorted_idx = curriculum.build_curriculum(
    train_data,
    seq_len=cfg.seq_len,
    cache_dir=cfg.cache_dir,
    alpha=cfg.curriculum_alpha,
    device=cfg.device,
  )
  ...
  dashboard = setup_dashboard(
    out_dir, f"enwik8 iter+curric ({n_params/1e6:.1f}M)", resume=bool(cfg.resume),
  )
  ...
  for phase in range(start_phase, cfg.n_layers):
    ...
    # Curriculum: how much of the dataset is unlocked at this phase
